...
$result3 = $db->query( "select * from teachers where teachers.id in (select id_teachers from timetable) order by teachers.id;" );  
...
function getData(&$db, $nTeacher, $nDay, $nTime)
{
   $query = "select 
   ...
   where timetable.id_teachers = $nTeacher and days_times.id_days=$nDay and days_times.id_times=$nTime";
   ...
   if( $db->count($result4) == 1)
   {
      $answer = "<font size=2 >".mysql_result( $result4, 0, "subject_name" )."<br>".
        ...
        mysql_result( $result4, 0, "group_name" )."</font>"
     ;
     return $answer; 
   }
...
for($nTeacher = 0; $nTeacher < $count3; $nTeacher++ )
{
   $idTeacher = mysql_result( $result3, $nTeacher, "id" );


   echo "<tr><td valign='middle' align='center' colspan=".($count1+1)."><td></tr>";

   echo "<tr><td valign='middle' align='center' bgcolor='#04B486' colspan=".($count1+1)."><b>".mysql_result( $result3, $nTeacher, "name" )."</b><td></tr>";
   // echo titles
   echo "<tr>";
   ...
   for($nTime = 0; $nTime < $count2; $nTime++)
   {
      ...
      for($nDay = 0; $nDay < $count1; $nDay++)
      {
        ...
        echo "<td valign='middle' align='center' bgcolor=".getColor($nTime + $nDay + 1).">".getData($db, $idTeacher, $idDay, $idTime)."</td>";
      }
      echo "</tr>";
   };
}
